...
通过 vibe tunnel，可以实现更便捷的远程编程体验：

- 在电脑上安装并配置 vibe tunnel 后，手机可以通过网络连接访问电脑终端
- 在浏览器中使用 Gemini 或 Claude Code 等程序，实现移动端的 vibe coding 体验
- 手机端可通过 EasyTier 等穿透工具建立与电脑的网络连接

**核心功能亮点：**

- **持久会话**：后台运行 iTerm2 等终端，相当于一个穷人的 tmux——通过网页随时查看，会话不会因断开连接而丢失
- **解决钥匙串问题**：SSH 直接连上来跑 Claude Code 时，往往需要先执行 `security unlock-keychain` 并输入密码来解开钥匙串；vibe tunnel 在本地跑，绕开了这个麻烦
- **知名作者背书**：作者之一是 [OpenClaw](https://github.com/openclaw) 的作者，另一位是 Flask 框架的作者 Armin Ronacher
